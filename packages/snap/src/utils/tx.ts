...
const hashTransaction = async (
  tx: any,
  encoding: ByteEncoding | 'bytes' = 'base58',
  includeSign = true
) => {
  ...
  if (tx.amount) {
    ...
    if (amountStr !== '') {
      ...
      if (amountUnit && amountUnit[1] !== 'aer') {
        throw Error(
          `Can only hash amounts provided in the base unit (aer), not '${tx.amount}'. Convert to aer or remove unit.`
        );
      }
      // Strip unit
      ...
      if (amount === '') {
        throw Error(
          `Could not parse numeric value from amount '${tx.amount}'.`
        );
      }
    }
  }
